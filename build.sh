...
for lang in en translations/??.po; do
	langcode=$(basename $lang .po)
	mkdir -p $langcode
	asciidoctor     -a imagesdir=../ -o $langcode/index.$langcode.html -a lang=$langcode index.$langcode.adoc
	asciidoctor-pdf -a media=print   -o $langcode/index.$langcode.pdf  -a lang=$langcode index.$langcode.adoc
